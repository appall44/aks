"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MaintenanceRequestController = void 0;
const common_1 = require("@nestjs/common");
const maintenance_service_1 = require("./maintenance.service");
const maintenance_request_dto_1 = require("./dto/maintenance-request.dto");
const tenant_service_1 = require("../tenant.service");
let MaintenanceRequestController = class MaintenanceRequestController {
    maintenanceService;
    tenantService;
    constructor(maintenanceService, tenantService) {
        this.maintenanceService = maintenanceService;
        this.tenantService = tenantService;
    }
    async createMaintenanceRequest(tenantId, dto) {
        const tenant = await this.tenantService.findById(tenantId);
        if (!tenant) {
            throw new common_1.NotFoundException(`Tenant with id ${tenantId} not found`);
        }
        return this.maintenanceService.createMaintenanceRequest(dto, tenant);
    }
    async getMaintenanceRequests(tenantId) {
        const tenant = await this.tenantService.findById(tenantId);
        if (!tenant) {
            throw new common_1.NotFoundException(`Tenant with id ${tenantId} not found`);
        }
        return this.maintenanceService.findByTenant(tenant);
    }
};
exports.MaintenanceRequestController = MaintenanceRequestController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Param)('tenantId', common_1.ParseIntPipe)),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, maintenance_request_dto_1.CreateMaintenanceRequestDto]),
    __metadata("design:returntype", Promise)
], MaintenanceRequestController.prototype, "createMaintenanceRequest", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Param)('tenantId', common_1.ParseIntPipe)),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], MaintenanceRequestController.prototype, "getMaintenanceRequests", null);
exports.MaintenanceRequestController = MaintenanceRequestController = __decorate([
    (0, common_1.Controller)('tenant/:tenantId/maintenance'),
    __metadata("design:paramtypes", [maintenance_service_1.MaintenanceRequestService,
        tenant_service_1.TenantService])
], MaintenanceRequestController);
//# sourceMappingURL=maintenance.controller.js.map