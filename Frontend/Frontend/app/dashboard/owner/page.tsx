...
useEffect(() => {
  ...
  async function fetchProperties() {
    try {
      ...
      const res = await axios.get<Property[]>(`${backendUrl}/owner/${ownerId}/properties`, {
...
useEffect(() => {
  async function fetchDashboardData() {
    try {
      ...
      const statsResp = await axios.get<DashboardStats>(`${backendUrl}/properties/dashboard`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setStats({
        propertiesCount: statsResp.data.totalProperties,
        totalRevenue: statsResp.data.totalRevenue,
        occupancyRate: statsResp.data.occupancyRate,
        activeTenants: statsResp.data.activeTenants,
      });

      const propertiesResp = await axios.get<Property[]>(`${backendUrl}/properties`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setProperties(propertiesResp.data);

      const activitiesResp = await axios.get<Activity[]>(`${backendUrl}/dashboard/owner/${ownerId}/activities`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setRecentActivities(activitiesResp.data);

      const performanceResp = await axios.get<PropertyPerformance[]>(`${backendUrl}/dashboard/owner/${ownerId}/property-performance`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setPropertyPerformance(performanceResp.data);
