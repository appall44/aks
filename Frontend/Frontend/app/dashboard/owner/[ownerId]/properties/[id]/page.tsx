...
export default function PropertyDetailsPage() {
 ...
 useEffect(() => {
  async function fetchData() {
    ...
    try {
      ...
      const [propertyRes, tenantsRes] = await Promise.all([
        ...
        axios.get<Tenant[]>(`${backendUrl}/properties/${id}/tenants`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);

      console.log('propertyRes.data:', propertyRes.data);
      console.log('tenantsRes.data:', tenantsRes.data);
  ...
  const occupiedUnits = property.occupiedUnits || tenants.length; 
  const occupancyRate = totalUnits > 0 ? Math.round((occupiedUnits / totalUnits) * 100) : 0;

  const recentActivities: any[] = []; 

  const handleViewTenant = (tenantId: number) => {
  alert(`View tenant id: ${tenantId}`);
};
  ...
  const handleDelete = async (tenantId: number) => {
	...
	return (
		<DashboardLayout>
          ...
          >
            <Link href={`/dashboard/owner/${id}/properties/${id}/units/new`}>
...
{property.images?.length ? (
  ...
  {((property.monthlyRevenue ?? 0) / 1000).toFixed(0)}K
</p>
							...
							<Card className="shadow-xl border-0 bg-white/95 backdrop-blur-sm">
								...
								<CardContent>
									<Table>
										<TableHeader>
											<TableRow>
												<TableHead>Tenant Full Name</TableHead>
...
{Array.isArray(tenants) && tenants.map((tenant) => (
 ...
 <TableCell>
  {`${tenant.firstname?.split(" ")[0] || "Unknown"} ${tenant.lastname || ""}`.trim()}
</TableCell>
    ...
    <TableCell>
      {tenant.monthlyRent || "N/A"}
    </TableCell>
    <TableCell>{tenant.status || "Approved"}</TableCell>

	<TableCell className="text-right">
        <button
          onClick={() => handleViewTenant(tenant.id)}
          className="text-blue-600 hover:text-blue-800"
          aria-label={`View details for ${tenant.firstname}`}
        >
          <FiEye size={20} />
        </button>
      </TableCell>
  ...
  {property?.monthlyRevenue != null ? `${property.monthlyRevenue.toLocaleString()} ETB` : "0 ETB"}
</span>
  ...
  {(property.pricePerUnit ?? 0).toLocaleString()} ETB
</span>
...
<span className="font-semibold text-emerald-600">
  {(property?.monthlyRevenue ?? 0).toLocaleString()} ETB
</span>
