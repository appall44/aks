...
const [moveInDate, setMoveInDate] = useState<Date | undefined>(undefined);
 ...
 useEffect(() => {
  ...
  if (!unit || !property) return <p>Loading unit and property data...</p>;

  const totalUpfront = unit.monthlyRent + unit.deposit;
  ...
  Apply to rent Unit {unit?.unitNumber} at {property?.name}
</p>
				...
				<form onSubmit={handleSubmit}>
									...
									<CardContent className="space-y-4">
										<div className="flex items-start space-x-3">
											...
											>
												...
												monthly rent of {unit.monthlyRent.toLocaleString()}{" "}
												ETB and security deposit of{" "}
												{unit?.deposit?.toLocaleString()} ETB. I understand
					...
					>
						<div className="flex justify-end space-x-4 mt-8">
							...
							>
								<Link
									href={`/dashboard/tenant/properties/${params.id}/units`}
								>
									Cancel
								</Link>
							</Button>
