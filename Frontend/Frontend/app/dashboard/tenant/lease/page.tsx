...
import {
  FileText, Download, Calendar, Home, User, Clock, MapPin
} from "lucide-react"
...
export default function TenantLeasePage() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/lease/LSE-001")
      .then(res => setLeaseInfo(res.data))
      .catch(err => console.error("Failed to load lease info", err))
  }, [])

  if (!leaseInfo) return <div className="p-10 text-gray-500">Loading lease details...</div>
  ...
  <CardContent>
    <div className="space-y-4">
      {leaseInfo.leaseTerms.map((term: string, index: number) => (
